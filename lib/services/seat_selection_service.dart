...
class _SeatSelectionScreenState extends State<SeatSelectionScreen> {
  ...
  void initState() {
    super.initState();
    _seatMap = _generateCleanSeatMap();
  ...
  /// Generate a clean seat map with no pre-booked seats
  List<List<Seat>> _generateCleanSeatMap() {
  ...
  /// Listen to Firestore for real-time booked seat updates
  void _listenToBookedSeats() {
    _seatSubscription = SeatService.streamBookedSeats(widget.showtime.id).listen((bookedLabels) {
      if (mounted) {
        setState(() {
          // Reset all seats to available first (except user's selections)
          for (var row in _seatMap) {
            for (var seat in row) {
          ...
          // Mark Firestore-booked seats
          for (var row in _seatMap) {
            for (var seat in row) {
              if (bookedLabels.contains(seat.label)) {
                seat.status = SeatStatus.booked;
                // If user had selected a seat that got booked by someone else, remove it
                _selectedSeats.removeWhere((s) => s.label == seat.label);
              }
            }
          }
          _isLoading = false;
  ...
  Widget build(BuildContext context) {
    ...
    final panelColor = isDark ? const Color(0xFF16213E) : Colors.white;
        ...
        children: [
            ...
            child: _isLoading
                ? Center(
                    child: Column(
                      ...
                      children: [
                        CircularProgressIndicator(color: isDark ? Colors.white24 : Colors.black26),
                        const SizedBox(height: 16),
                        Text('Đang tải ghế...', style: TextStyle(color: subtextColor, fontSize: 13)),
                      ],
                    ),
                  )
                : InteractiveViewer(
          ...
          Padding(
            ...
            child: Row(
              mainAxisAlignment: MainAxisAlignment.center,
              children: [
                _legendItem(isDark ? Colors.white.withOpacity(0.08) : Colors.black.withOpacity(0.06), isDark ? Colors.white24 : Colors.black12, 'Trống'),
                const SizedBox(width: 16),
                _legendItem(Colors.redAccent, Colors.redAccent, 'Đang chọn'),
                const SizedBox(width: 16),
                _legendItem(isDark ? Colors.white.withOpacity(0.03) : Colors.black.withOpacity(0.03), isDark ? Colors.white10 : Colors.black12, 'Đã bán'),
                const SizedBox(width: 16),
              ...
              child: Column(
                ...
                children: [
                  ...
                  if (_selectedSeats.isNotEmpty) ...[
                    Row(
                      children: [
                        Text('Ghế: ', style: TextStyle(color: subtextColor, fontSize: 14)),
                        Expanded(
                          child: Text(
                  ...
                  SizedBox(
                    ...
                    child: ElevatedButton(
                      style: ElevatedButton.styleFrom(
                        backgroundColor:
                            _selectedSeats.isEmpty ? isDark ? Colors.white12 : Colors.black12 : Colors.redAccent,
                      ...
                      child: Text(
                        ...
                        style: TextStyle(
                          color: _selectedSeats.isEmpty ? isDark ? Colors.white24 : Colors.black12 : Colors.white,
                          fontSize: 16,
                          fontWeight: FontWeight.bold,
                        ),
                      ),
  ...
  Color _seatColor(Seat seat) {
    ...
    switch (seat.status) {
      ...
      case SeatStatus.booked:
        return isDark ? Colors.white.withOpacity(0.03) : Colors.black.withOpacity(0.03);
